...
struct CubeRefPointPair {
    ...
    CubeRefPointPair(decltype(Point::x) x, decltype(Point::y) y) : point(x, y), cube(cubes2dArr[x][y]) {}
};
    ...
    auto tryAddNeighbor = [&](const Point &neighborPoint) {
        if (neighborPoint.isValid() == true) {
            ...
            if (neighborCube.type == Cube::type_to_beaten_type(currentType)) {
                SPDLOG_TRACE("Exploaded cube (x, y) = [{}, {}] with type = [{}], neighbor cube (x, y) = [{}, {}] with "
                             "type = [{}] gets destroyed",
                             cubePoint.x, cubePoint.y, Cube::type_to_string(currentType), neighborPoint.x,
                             neighborPoint.y, Cube::type_to_string(neighborCube.type));
...
bool getDestructibleCubePoints(std::vector<Point> &destructiblePoints_out, bool isUsedForReplacement)
{
    ...
    for (auto row = 0; row < cubes2dArr.size(); row++) {
        for (auto column = 0; column < cubes2dArr[0].size(); column++) {
            ...
            if (horizontalDestructiblePoints.size() >= 3) {
                destructiblePoints_out.insert(destructiblePoints_out.end(), horizontalDestructiblePoints.begin(),
                                              horizontalDestructiblePoints.end());
            }

            if (verticalDestructiblePoints.size() >= 3) {
                destructiblePoints_out.insert(destructiblePoints_out.end(), verticalDestructiblePoints.begin(),
                                              verticalDestructiblePoints.end());
            }
        }
    }
    ...
    if (destructiblePoints_out.size() >= 3) {
        ...
        destructiblePoints_out.erase(std::unique(destructiblePoints_out.begin(), destructiblePoints_out.end()),
                                     destructiblePoints_out.end());
        ...
        if (isUsedForReplacement != true) {
            ...
            if (specialDestructiblePoints.size() > 0) {
                destructiblePoints_out.insert(destructiblePoints_out.end(), specialDestructiblePoints.begin(),
                                              specialDestructiblePoints.end());
                std::sort(destructiblePoints_out.begin(), destructiblePoints_out.end());
                destructiblePoints_out.erase(std::unique(destructiblePoints_out.begin(), destructiblePoints_out.end()),
                                             destructiblePoints_out.end());
            }
...
void createNewGame(void)
{
    for (auto row = 0; row < cubes2dArr.size(); row++) {
        for (auto column = 0; column < cubes2dArr[0].size(); column++) {
            ...
            SPDLOG_TRACE("First pass assigning cube (x, y) = [{}, {}] with type = [{}]", row, column,
                         Cube::type_to_string(cubes2dArr[row][column].type));
        }
    }
